 ...
 */
function depenetrateAlongNormal(mothership, shipCollisionPointsAt, x, y, nx, ny, shipRadius){
...
export function resolveMothershipCollisionResponse(args){
  ...
  if (overlapBefore){
    const poseContact = nearestBoundaryContactAtPose(mothership, shipCollisionPointsAt, ship.x, ship.y) || (() => {
      const n = fallbackGradientNormal((x, y) => collision.airValueAtWorld(x, y), eps, ship.x, ship.y, hx, hy);
      return { x: hx, y: hy, nx: n.nx, ny: n.ny, edgeIdx: -1 };
    })();
    const depen = depenetrateAlongNormal(mothership, shipCollisionPointsAt, ship.x, ship.y, poseContact.nx, poseContact.ny, shipRadius);
    ship.x = depen.x;
    ship.y = depen.y;
